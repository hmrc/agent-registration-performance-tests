...
class AgentRegistrationProvideDetailsContentionSimulation extends Simulation {

  private val provideDetailsFeeder =
    csv("data/provide-details-concurrency.csv").queue

  private val httpProtocol =
    http.disableFollowRedirect

  private val provideDetailsContentionScenario =
    scenario("Six individuals provide details on the same application at the same time")
      .feed(provideDetailsFeeder)
      .exec(
  ...
  setUp(
    provideDetailsContentionScenario.inject(
      atOnceUsers(6)
    )
  )
